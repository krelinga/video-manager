...
func (c *Client) PatchMovieEditionWork(ctx context.Context, workUUID uuid.UUID, patch *catalog.MovieEditionWorkPatch) (*catalog.Work, error) {
	...
	if err := row.Scan(&kind, &rawBody); errors.Is(err, pgx.ErrNoRows) {
		...
		return nil, fmt.Errorf("%w: work %s is not a movie edition work", catalog.ErrKind, workUUID)
	}
	...
	return &catalog.Work{
		UUID:             workUUID,
		MovieEditionWork: catalog.NewOptPtr(body.ToPublic()),
	}, nil
}
