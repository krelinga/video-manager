...
func (c *Client) PatchExtraWork(ctx context.Context, workUUID uuid.UUID, patch *catalog.ExtraWorkPatch) (*catalog.Work, error) {
	...
	} else if kind != workKindExtra {
		return nil, fmt.Errorf("%w: work %s is not an extra work", catalog.ErrKind, workUUID)
	}
	...
	return &catalog.Work{
		UUID:      workUUID,
		ExtraWork: catalog.NewOptPtr(body.ToPublic()),
	}, nil
}
